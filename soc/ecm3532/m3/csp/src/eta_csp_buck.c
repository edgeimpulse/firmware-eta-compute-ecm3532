...
BuckGetFlashInfoFreqOffsets(int8_t *i8M3FreqOffset, int8_t *i8DspFreqOffset)
{
    ...
    // Confirm that the data is valid
    //
    if (ui32InfoData[FLASH_INFO_BUCK_FREQ_OFFSET_KEY_INDEX] == 0x99996666) {
...
// 
//     ui32InfoPage[FLASH_INFO_BUCK_FREQ_OFFSET_KEY_ADDRESS >> 2]   = 0x99996666;
...
BuckGetDspVoltage(void)
{
    ...
    //
    sregRtcPwr.V = REG_RTC_PWR.V;
    if (sregRtcPwr.BF.DSP_BUCK == 0) {
        return (eBuckDspOff);
    }
                          ...
                          tBuckWait4Stable  iWaitForStable)
{
    ...
    //
    // If this function was called with frequency = 0, error
    //
    if (ui32DspFrequency == 0) {
...
EtaCspBuckPrePowerDown(void)
{
    ...
    EtaCspBuckAoVoltageSet(eBuckAo600Mv, false);
    EtaCspBuckMemVoltageSet(eBuckMem900Mv, true);
...
//    bool                    bFreqTargetsValid;
    ...
    //
//    bFreqTargetsValid = BuckGetFlashInfoFreqOffsets(&(sregBuckShadow.i8M3FreqOffset), 
//                                                    &(sregBuckShadow.i8DspFreqOffset) );
    ...
    //
    // Need to determine the freq_target offsets if they are not 
    // already stored in flash
    //
// Don't do this until we resolve issue with determining offset for the bucks
//    if (!bFreqTargetsValid) {
//         // Go calculate the offsets
//         if (!BuckCalculateFreqTargets()) {     
//             return (eEtaBuckInvalidFreqOffsets);
//         }
//    }
