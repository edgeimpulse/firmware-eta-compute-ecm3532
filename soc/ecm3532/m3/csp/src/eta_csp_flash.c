		   ...
		   bool bBkpt)
{
    ...
    //
    uint32_t pui32Buffer[ETA_CSP_FLASH_PAGE_SIZE >> 2];
    ...
    {
        IFDBG EtaCspIoPrintf("\r\n Mei: address < BASE \r\n");
        return(eEtaFlashAddressError+0);
    }
    if(ui32Address >= ETA_CSP_FLASH_MAX)
    {
        IFDBG EtaCspIoPrintf("\r\n Mei: address 0x%x > MAX 0x%x\r\n", ui32Address, ETA_CSP_FLASH_MAX);
        return(eEtaFlashAddressError+1);
    }
    if(ui32Count > ETA_CSP_FLASH_PAGE_SIZE)
    {
        IFDBG EtaCspIoPrintf("\r\n Mei: count > PAGE_SIZE \r\n");
    ...
    if(ui32Count > ETA_CSP_FLASH_PAGE_SIZE)
    {
        return(eEtaFlashPageSizeError+1);
    }
    ...
    if(ui32Work > ETA_CSP_FLASH_PAGE_SIZE)
    {
        IFDBG EtaCspIoPrintf("\r\n Mei: cross page boundary \r\n");
        return(eEtaFlashAddressError+2);
    }

    //
    // IFDBG EtaCspIoPrintf("EtaCspFlashProgram: ui32Address = 0x%x ui32Count = %d
    // ui32Address + ui32Count =
    // 0x%x\r\n",ui32Address,ui32Count,ui32Address+ui32Count);
    ...
    //
    // For now, make sure we start at the beginning of a page.
    //
    // if((ui32Address & ~ETA_CSP_FLASH_PAGE_SIZE_MASK) != 0x0)
    // {
    ...
    for(i32I = 0; i32I < (ETA_CSP_FLASH_PAGE_SIZE>>2); i32I++)
    {
    ...
    IFDBG EtaCspIoPrintf("%s Line %d 0x%x %d 0x%x \r\n",__FILE__, __LINE__,
    ...
    IFDBG EtaCspIoPrintf("Line %d 0x%x 0x%x 0x%x 0x%x 0x%x \r\n", __LINE__,
                         pui32Buffer[0], pui32Buffer[1], pui32Buffer[2],
                         pui32Buffer[3], pui32Buffer[4]);
    ...
    if(bBkpt)
    {
        pfnFlashBrProgramBkpt((ui32Address & ETA_CSP_FLASH_PAGE_SIZE_MASK),
                          ...
                          ETA_CSP_FLASH_PAGE_SIZE >> 2, // number of 32-bit words
 ...
 *        greater than ETA_CSP_FLASH_PAGE_SIZE.
 *
 ******************************************************************************/
tEtaStatus
...
{
    uint64_t pui64Buffer[ETA_CSP_FLASH_PAGE_SIZE >> 3]; // 512 64-bit words
                                                        // aligned 64-bits
    ...
    for(i32I = 0; i32I < ETA_CSP_FLASH_PAGE_SIZE; i32I++)
    {
        *pui8Ptr++ = 0xFF;
    }
    pui8Ptr = (uint8_t *)pui64Buffer;
...
EtaCspFlashEtaInfoBytesAreErased(void)
{
    ...
    for(ii=0;ii<(ETA_CSP_FLASH_PAGE_SIZE >> 2);ii+=4){
...
EtaCspFlashProgramFast(uint32_t ui32Address, uint32_t *pui32Src)
{
    ...
    if((ui32Address+ETA_CSP_FLASH_PAGE_SIZE) > ETA_CSP_FLASH_MAX)
    {
        return(eEtaFlashAddressError+1);
    }
    ...
    ui32Rc = pfnFlashBrProgram((ui32Address & ETA_CSP_FLASH_PAGE_SIZE_MASK),
                               pui32Src,
                               ETA_CSP_FLASH_PAGE_SIZE >> 2, // # 32-bit words
                               0, // program normal space
                               ETA_CSP_FLASH_TNVS_COUNT,
                               ETA_CSP_FLASH_TPGS_COUNT,
                               ETA_CSP_FLASH_TPROG_COUNT,
