...
#define LSM_FFT_DSP_DRV_NAME  CONFIG_LSM6DSL_FFT_DSP_NAME
...
static int32_t lsmCompoDspDrvProbe(void);
static tDrvHandle lsmCompoDspDrvOpen(char* cDrvName);
static int32_t lsmCompoDspDrvSetEvCb(tDrvHandle sDrvHandle,
                                tSensorType iSensor, tSensorEventCb fEvCb,
                                tSensorEvent iEventMask, void* vDrvdata);
static int32_t lsmCompoDspDrvIoctl(tSDrvHandle sDrvHandle, tSensorType iSensor,
                                tSensorIOCTL iCmd, void* vPtr);
static int32_t lsmCompoDspDrvRead(tDrvHandle sDrvHandle, tSensorType iSensor,
                                uint8_t* ui8Buf, uint8_t ui8Len);

tSensorDrvOps lsmCompoDspDrvops = {
    .DrvProbe = lsmCompoDspDrvProbe,
    .DrvOpen = lsmCompoDspDrvOpen,
    //    .DrvClose = lsmCompoDspDrvClose,
        .DrvRead = lsmCompoDspDrvRead,
        .DrvIoctl = lsmCompoDspDrvIoctl,
        .DrvSetCb = lsmCompoDspDrvSetEvCb,
};
...
int Lsm6dslCompoDspDriverInit(void)
{
    ...
    SET_SENSOR_ID(eventHeaderMask, LSM6DSL_FFT_SENSOR_ID);
   ...
   // register with Sensor driver subsystem
    if (SensorDrvRegister(LSM_FFT_DSP_DRV_NAME, &lsmCompoDspDrvops)) {
        //TBD: Add trace message of error
        return -1;
    }
    return 0;
...
static int32_t lsmCompoDspDrvProbe(void)
{
    ...
    sensorCallParams->sensorHwId = LSM6DSL_FFT_SENSOR_ID;
...
static int32_t lsmCompoDspDrvOpen(char* cDrvName)
{
    tSensorCallParams* sensorCallParams;
    int32_t status = 0;
    sensorCallParams = SharedMemAlloc(sizeof(tSensorCallParams));
    ...
    //  or should we send Something like LSM6DSL_FFT_SENSOR_ID +1.TBD: based on that set handle variable.
    SharedMemFree(sensorCallParams);
    xSemaphoreGive(xLsmFnMutex);
    return status;
}
...
//Read
static int32_t lsmCompoDspDrvRead(tDrvHandle sDrvHandle, tSensorType iSensor, uint8_t* ui8Buf, uint8_t ui8Len)
{
    tSensorCallParams* sensorCallParams;
...
static int32_t lsmCompoDspDrvSetEvCb(tDrvHandle sDrvHandle, tSensorType iSensor, tSensorEventCb fEvCb, tSensorEvent iEventMask, void* vDrvdata)
{
    int index = 0;
