...
EtaCspI2sConfig(tI2sConfig i2sConfig)
{
  ...
  ui32MboxWord = i2sConfig.packed_config;
  ...
  uint32_t enable_rx       = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_RX_ENBLE) >> BP_ETA_CSP_COMMON_I2S_RX_ENBLE);
  uint32_t enable_tx       = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_TX_ENBLE) >> BP_ETA_CSP_COMMON_I2S_TX_ENBLE);
  //uint32_t swap_chan_tx    = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_TX_SWP_C) >> BP_ETA_CSP_COMMON_I2S_TX_SWP_C);
  //uint32_t swap_chan_rx    = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_RX_SWP_C) >> BP_ETA_CSP_COMMON_I2S_RX_SWP_C);
  uint32_t mode_64bit      = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_64B_MODE) >> BP_ETA_CSP_COMMON_I2S_64B_MODE);
  //uint32_t edge_tx_trig    = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_TX_ED_TR) >> BP_ETA_CSP_COMMON_I2S_TX_ED_TR);
  //uint32_t edge_rx_trig    = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_RX_ED_TR) >> BP_ETA_CSP_COMMON_I2S_RX_ED_TR);
  uint32_t clock_src       = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_CLCK_SRC) >> BP_ETA_CSP_COMMON_I2S_CLCK_SRC);
  uint32_t sample_freq     = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_SMPL_FRQ) >> BP_ETA_CSP_COMMON_I2S_SMPL_FRQ);
  //uint32_t chan_tx_config  = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_TX_CONFG) >> BP_ETA_CSP_COMMON_I2S_TX_CONFG);
  //uint32_t chan_rx_config  = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_RX_CONFG) >> BP_ETA_CSP_COMMON_I2S_RX_CONFG);
  //uint32_t chan_tx_upscale = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_TX_UPSCL) >> BP_ETA_CSP_COMMON_I2S_TX_UPSCL);
  //uint32_t chan_rx_upscale = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_RX_UPSCL) >> BP_ETA_CSP_COMMON_I2S_RX_UPSCL);
  //uint32_t int_mode        = ((ui32MboxWord & BM_ETA_CSP_COMMON_I2S_INT_MODE) >> BP_ETA_CSP_COMMON_I2S_INT_MODE);
  ...
  if (enable_tx && enable_rx) // Enable BCLK, LRCLK, DATAOUT, DATAIN
  {
    #ifdef ETA_CSP_I2S_DEBUG_PRINT
  ...
  else if (enable_rx) // Enable BCLK, LRCLK, DATAIN
  {
    #ifdef ETA_CSP_I2S_DEBUG_PRINT
  ...
  else if (enable_tx) // Enable BCLK, LRCLK, DATAOUT
  {
    #ifdef ETA_CSP_I2S_DEBUG_PRINT
...
//  while (REG_I2S_CSR2.V == BM_I2S_CSR2_SFTRST);
  ...
  REG_I2S_CSR2.V = (mode_64bit     << BP_I2S_CSR2_BIT64_MODE)  |
                   (sample_freq    << BP_I2S_CSR2_SAMPLE_FREQ) ;
