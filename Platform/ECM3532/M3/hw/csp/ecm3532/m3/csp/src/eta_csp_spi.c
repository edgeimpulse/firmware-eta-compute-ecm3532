...
/***************************************************************************//**
 *
 * EtaCspSpiInit - Initialize the SPI module. Ensure that
 *   EtaCspSocCtrlUstFreqSet is called BEFORE calling this function.
 *
 *  @param iNum the SPI number.
 *  @param tSpiConfig SPI Configurtation struct
 *  @return The result.
 *
 ******************************************************************************/
tEtaStatus
EtaCspSpiInit(tSpiNum iNum, tSpiConfig iSpiConfig)
{
...
volatile tSpiCmd * g_pEtaCspSpiCmd[2];


/***************************************************************************//**
 *
 *  EtaCspSpiIntStatusGet - Get interrupt status structure
 *
 *  @param iNum the SPI number.
 *  @return tSpiInt struct of interrupts triggered.
 *
 ******************************************************************************/
tSpiInt EtaCspSpiIntStatusGet(tSpiNum iNum)
{
    reg_spi_int_act_t reg_spi_int_act;
    reg_spi_int_act.V = MEMIO16(REG_SPI_INT_ACT_ADDR(iNum) );

    tSpiInt iSpiIntStatus = {
        .bCmdIsDone      = reg_spi_int_act.BF.CMD_IS_DONE_ACT,
        .bXmitComplete   = reg_spi_int_act.BF.XMIT_COMPLETE_ACT,
        .bRxFifoHitLevel = reg_spi_int_act.BF.RX_FIFO_HIT_LEVEL_ACT,
        .bTxFifoHitLevel = reg_spi_int_act.BF.TXFIFO_HAS_LEVEL_SPACE_ACT,
        .bRxFifoHasData  = reg_spi_int_act.BF.RX_DATA_ACT,
        .bTxFifoHasSpace = reg_spi_int_act.BF.TX_DATA_ACT
    };

    return iSpiIntStatus;
}


/***************************************************************************//**
 *
 *  EtaCspSpiIntSet - Set interrupts enable bits
 *
 *  @param iNum the SPI number.
 *  @param iSpiInt interrupt structure.
 *  @return void
 *
 ******************************************************************************/
void EtaCspSpiIntSet(tSpiNum iNum, tSpiInt iSpiInt)
{
    reg_spi_int_en_t reg_spi_int_en;
    reg_spi_int_en.V = MEMIO16(REG_SPI_INT_EN_ADDR(iNum) );

    reg_spi_int_en.BF.CMD_IS_DONE_EN0            = iSpiInt.bCmdIsDone;
    reg_spi_int_en.BF.XMIT_COMPLETE_EN0          = iSpiInt.bXmitComplete;
    reg_spi_int_en.BF.RX_FIFO_HIT_LEVEL_EN0      = iSpiInt.bRxFifoHitLevel;
    reg_spi_int_en.BF.TXFIFO_HAS_LEVEL_SPACE_EN0 = iSpiInt.bTxFifoHitLevel;
    reg_spi_int_en.BF.RX_DATA_EN0                = iSpiInt.bRxFifoHasData;
    reg_spi_int_en.BF.TX_DATA_EN0                = iSpiInt.bTxFifoHasSpace;

    MEMIO16(REG_SPI_INT_EN_ADDR(iNum) ) = reg_spi_int_en.V;
}


/***************************************************************************//**
 *
 *  EtaCspSpiIntEnable - Enable interrupts with bits set to TRUE
 *
 *  @param iNum the SPI number.
 *  @param iSpiInt interrupts set to TRUE are enabled.
 *  @return void
 *
 ******************************************************************************/
void EtaCspSpiIntEnable(tSpiNum iNum, tSpiInt iSpiInt)
{
    reg_spi_int_en_t reg_spi_int_en;
    reg_spi_int_en.V = MEMIO16(REG_SPI_INT_EN_ADDR(iNum) );
...
/***************************************************************************//**
 *
 *  EtaCspSpiIntDisable - Disable interrupts with bits set to TRUE
 *
 *  @param iNum the SPI number.
 *  @param iSpiInt interrupts set to TRUE are disabled.
 *  @return void
 *
 ******************************************************************************/
void EtaCspSpiIntDisable(tSpiNum iNum, tSpiInt iSpiInt)
{
    reg_spi_int_en_t reg_spi_int_en;
    reg_spi_int_en.V = MEMIO16(REG_SPI_INT_EN_ADDR(iNum) );
...
/***************************************************************************//**
 *
 *  EtaCspSpiIntGet - Get interrupts enable bits
 *
 *  @param iNum the SPI number.
 *  @return iSpiInt interrupt structure.
 *
 ******************************************************************************/
tSpiInt EtaCspSpiIntGet(tSpiNum iNum)
{
    reg_spi_int_en_t reg_spi_int_en;
    reg_spi_int_en.V = MEMIO16(REG_SPI_INT_EN_ADDR(iNum) );

    tSpiInt iSpiIntStatus = {
        .bCmdIsDone      = reg_spi_int_en.BF.CMD_IS_DONE_EN0,
        .bXmitComplete   = reg_spi_int_en.BF.XMIT_COMPLETE_EN0,
        .bRxFifoHitLevel = reg_spi_int_en.BF.RX_FIFO_HIT_LEVEL_EN0,
        .bTxFifoHitLevel = reg_spi_int_en.BF.TXFIFO_HAS_LEVEL_SPACE_EN0,
        .bRxFifoHasData  = reg_spi_int_en.BF.RX_DATA_EN0,
        .bTxFifoHasSpace = reg_spi_int_en.BF.TX_DATA_EN0
    };
...
/***************************************************************************//**
 *
 *  EtaCspSpiSlaveRxDataReceive - Receive bytes from the master. Function will
 *    return fewer than the requested number of bytes if the RX buffer becomes
 *    empty.
 *
 *  @param iNum the SPI number.
 *  @param pui8RxData pointer to RX Data Array
 *  @param ui32RxReadLen RX Data bytes to read from the interface
 *  @param pui32RxRecLen RX Data bytes actually received on return
 *  @return tEtaStatus will return eEtaSpiRxFifoOflow if the RX FIFO overflows
 *
 ******************************************************************************/
                        ...
                        uint32_t ui32RxReadLen, uint32_t * pui32RxRecLen)
{
    ...
    uint8_t ui8Debug = MEMIO8(REG_SPI_RX_FIFO_DEBUG_ADDR(iNum) );
    int32_t i32Entries = ui8Debug & BM_SPI_RX_FIFO_DEBUG_ENTRIES;

    // Set the number of received bytes based on the number in the FIFO.
    *pui32RxRecLen = ui32RxReadLen;
    if (i32Entries < ui32RxReadLen) {
        *pui32RxRecLen = i32Entries;
    }
    else if (i32Entries > ui32RxReadLen) {
        i32Entries = ui32RxReadLen;
    }
    ...
    while (i32Entries > 0) {
        switch (i32Entries) {
            case 0:
                ...
                break;
            case 1:
                *pui8RxData = MEMIO8(REG_SPI_TXRX_1BYTE_FIFO_DATA_ADDR(iNum) );
                pui8RxData += 1;
                i32Entries -= 1;
                break;
            case 2:
            case 3:
                *((uint16_t*)pui8RxData) = 
                    MEMIO16(REG_SPI_TXRX_2BYTE_FIFO_DATA_ADDR(iNum) );
                pui8RxData += 2;
                i32Entries -= 2;
                break;
            default:
                *((uint32_t*)pui8RxData) = REG_SPI_RX_4BYTE_FIFO_DATA(iNum).V;
                pui8RxData += 4;
                i32Entries -= 4;
                break;
        }
    ...
    ui8Debug = MEMIO8(REG_SPI_RX_FIFO_DEBUG_ADDR(iNum) );
    if (ui8Debug & BM_SPI_RX_FIFO_DEBUG_OFLOW) {
        iStatus = eEtaSpiRxFifoOflow;
    }
...
/***************************************************************************//**
 *
 *  EtaCspSpiSlaveTxDataSend - Send bytes to the master. Function will send
 *    fewer than the requested number of bytes if the TX buffer becomes full.
 *
 *  @param iNum the SPI number.
 *  @param pui8TxData pointer to TX Data Array
 *  @param ui32TxWriteLen TX Data bytes to write to the interface
 *  @param pui32TxSentLen TX Data bytes actually sent on return
 *  @return tEtaStatus will return eEtaSpiTxFifoUflow if the TX FIFO underflows
 *
 ******************************************************************************/
                        ...
                        uint32_t ui32TxWriteLen, uint32_t * pui32TxSentLen)
{
    ...
    uint8_t ui8Debug = MEMIO8(REG_SPI_TX_FIFO_DEBUG_ADDR(iNum) );
    int32_t i32Space = 16 - (ui8Debug & BM_SPI_TX_FIFO_DEBUG_ENTRIES);

    // Set the number of sent bytes based on the number in the FIFO.
    *pui32TxSentLen = ui32TxWriteLen;
    if (i32Space < ui32TxWriteLen) {
        *pui32TxSentLen = i32Space;
    }
    else if (i32Space > ui32TxWriteLen) {
        i32Space = ui32TxWriteLen;
    }
    
    // Put data in the FIFO
    for (/* NONE */; i32Space > 0; i32Space--) {
        MEMIO8(REG_SPI_TXRX_1BYTE_FIFO_DATA_ADDR(iNum) ) = (*pui8TxData++);
    }

    ui8Debug = MEMIO8(REG_SPI_TX_FIFO_DEBUG_ADDR(iNum) );
    if (ui8Debug & BM_SPI_TX_FIFO_DEBUG_UFLOW) {
        iStatus = eEtaSpiTxFifoUflow;
    }
