...
static int64_t i64EtaCspTSenseSlope; // in upscaled deci-degrees per Hz
...
EtaCspTSenseInit(void)
{
    ...
    //
    // Compute the slope in deci-degrees per Hz, then upscale
    //
// fixme when the algorithm solidifies.
    i64EtaCspTSenseSlope  = ((i32EtaCspTSenseTemperatureA -(ANA_SIM_INTERCEPT_TEMP)) * 10) * 65536L;
...
EtaCspTSenseGet (void)
{
    ...
    int64_t  i64Temperature = i32Measured * i64EtaCspTSenseSlope; // upscaled deci-degrees
    ...
    //
    i64Temperature  /= 65536; // deci-degrees
